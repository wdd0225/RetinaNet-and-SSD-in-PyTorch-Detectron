...
class COCODetection(data.Dataset):
    ...
    def __init__(self, root, image_sets='val2017', preproc=None, target_transform=None,
                 dataset_name='COCO'):
        self.root = root
        self.cache_path = os.path.join('../coco/', 'cache')
        ...
        self.coco = COCO(os.path.join(cfg.COCO_ROOT, cfg.ANNOTATIONS,
                                  cfg.INSTANCES_SET.format(image_sets)))

        for image_set in [image_sets]: #FIXME: currently only support one dataset to use
            coco_name = image_set 
    ...
    def pull_item(self, index):
        ...
        # full_root = os.path.join('/mnt/lustre/DATAshare/detection_dataset/coco2017/', 'images', 'val2017')
        # path = os.path.join(full_root, self.coco.loadImgs(img_id)[0]['file_name'])
        # print('XXXXXXXX: ',path)
    ...
    def _print_detection_eval_metrics(self, coco_eval):
        ...
        for cls_ind, cls in enumerate(self._classes):
            ...
            test_num=test_num+1
            print('{:d}: {:.1f}'.format(test_num,100 * ap))
            
        print('number of class:  ', test_num)
