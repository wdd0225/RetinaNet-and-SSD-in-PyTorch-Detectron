...
class RoiDataLoader(data.Dataset): # the batch sampler
...
def collate_minibatch(list_of_blobs):
    ...
    '''Seems like each time only two images are passed in, so the loop is only called once, but function is called for each batch of two images'''
    if cfg.RPN.RPN_ON:
...
def pad_key_in_blobs(list_of_blobs):
    for k in list_of_blobs[0]:
        if not (k.find('retnet_roi_bbox_targets' ) >= 0  or k.find('retnet_roi_fg_bbox_locs') >= 0): #becasue (4,) maybe returned if only one found
            continue
