...
def get_image_blob(im, target_scale, target_max_size):
    ...
    they were trained with， fixed value as np.array([[[102.9801, 115.9465, 122.7717]]])
    '''
    '''returns a pair list of resized images with scale ratios'''

    processed_im, im_scale = prep_im_for_blob(
        im, cfg.PIXEL_MEANS, [target_scale], target_max_size
    )
    
    '''the format will be (batch elem index, channel, height, width)'''
...
def im_list_to_blob(ims):
    ...
    if not isinstance(ims, list):
        ims = [ims]
    '''max possible h and max possible w, if in FPN, then also consider divisible by stride, so could be smaller'''
    max_shape = get_max_shape([im.shape[:2] for im in ims])
    
    num_images = len(ims)
    '''num_of_images, max_height, max_width and num_of_channels'''
    
    blob = np.zeros(
        (num_images, max_shape[0], max_shape[1], 3), dtype=np.float32)
    for i in range(num_images):
        im = ims[i]
        '''store all resized images in a fixed max-height-and-weight np array'''
...
def get_max_shape(im_shapes):
    ...
    """
    '''
    axis=0 will take max of x,y from all (x,y), axis=1 will take each of max among x, y,
    thus in this case it returns the largest h and w of any images
    '''
...
def prep_im_for_blob(im, pixel_means, target_sizes, max_size):
    ...
    """
    '''im, cfg.PIXEL_MEANS, [target_scale], target_max_size, where target_scale is merely cfg.TEST.SCALE'''
    ...
    for target_size in target_sizes:
        '''
        get_target_scale returns a ratio, which adjust the image size, so that new size < max_size must be True, min size = target_size could be better
        but whatever.
        '''
