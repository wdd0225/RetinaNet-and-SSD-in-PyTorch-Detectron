...
def process_in_parallel(
    ...
    """
    '''
    From boss owrker of test_engine:
    ...
    for i, gpu_ind in enumerate(gpu_inds):
        ...
        '''
        it shows that the binary file should be the test_net.py itself,
        so the worker processes are all calling the script again,
        and then somehow realize they are worker instead of a boss
        '''
        ...
        '''it seems like it is already running here, which means the worker threads are starting here'''
        p = subprocess.Popen(
            cmd,
            shell=True,
            env=subprocess_env,
    ...
    for i, p, start, end, subprocess_stdout in processes:
        '''
        seems like it outputs a temp file for each process and this file is read by the main process
        just a guess, if only one thread, the no need to enter here, just work as single thread;
        but if mutli thread, then the main thread will enter here and call a ranged script for each worker thread,
        then each worker will not enter here, they will go to child_func and their outputs are collected by the 
