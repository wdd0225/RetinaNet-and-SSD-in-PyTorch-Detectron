...
'''when infering, only fill in model, image and timer'''
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    '''
    tic and toc are merely starting and ending timing
    while the dict timers contain several timer objects, key=im_detect_bbox, misc_bbox
    '''
    timers['im_detect_bbox'].tic()
    '''Enable test-time augmentation for bounding box detection if True, in our case it turns out to be false'''
    if cfg.TEST.BBOX_AUG.ENABLED:
        scores, boxes, im_scale, blob_conv = im_detect_bbox_aug(
            model, im, box_proposals)
    else:
        '''
        test.scale: The scale is the pixel size of an image's shortest side
        test.max_size: Max pixel size of the longest side of a scaled input image
        '''
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None):
    ...
    '''
    blobs: data:        np matrix of (batch elem index, channel, height, width)
           im_info:     height, width and resize ratio
           roi:         in this case should not exist
    im_scale:           resize ratio
    '''
    inputs, im_scale = _get_blobs(im, boxes, target_scale, target_max_size)

    '''we are faster rcnn, so skip this'''
    ...
    '''
    this is calling forward function of generalized_rcnn, which internally call _forward
    which calls conv_body and RPN inside. This should be a great discovery
    inside the forward they also call forward of fast_rcnn_heads
    '''
...
def _get_blobs(im, rois, target_scale, target_max_size):
    ...
    '''im, boxes, target_scale, target_max_size'''
    blobs = {}
    '''
    blob_utils is from utils.blob, get_image_blob converts an image into a network input.
    returns: 
    blob is a np matrix of (batch elem index, channel, height, width), 
    im_scale contains resize ratio of each image
    im_info contains height, width, resize_ratio
    '''
    blobs['data'], im_scale, blobs['im_info'] = \
        blob_utils.get_image_blob(im, target_scale, target_max_size)
    if rois is not None:
        '''in detect_image_bbox case, the box is passed but =None'''
