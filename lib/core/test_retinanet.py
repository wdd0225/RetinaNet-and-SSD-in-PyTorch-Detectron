...
#     box_preds = workspace.FetchBlobs(box_preds)
    ...
    for lvl in range(k_min, k_max + 1):
        ...
        inds = np.argpartition(
            cls_prob_ravel[candidate_inds], -pre_nms_topn)[-pre_nms_topn:] # select thos better than threshold and top 10000
        inds = candidate_inds[inds]

        inds_5d = np.array(np.unravel_index(inds, cls_prob.shape)).transpose() #to transform index according to data shape
