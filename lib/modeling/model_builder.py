...
def get_func(func_name):
    ...
    'modeling' module.
    """
    '''FPN.fpn_ResNet50_conv5_body is an example passing in'''
    '''fast_rcnn_heads.roi_2mlp_head is another'''
    if func_name == '':
        return None
    try:
...
#         assert cfg.FPN.RPN_MAX_LEVEL >= cfg.FPN.ROI_MAX_LEVEL
    ...
    def _forward(self, data, im_info, roidb=None, **rpn_kwargs):
        ...
        # if self.training and not cfg.RETINANET.RETINANET_ON:
        ...
        if self.training:
            ...
            if cfg.FPN.FPN_ON:
                for i, lvl in enumerate(range(cfg.FPN.RPN_MIN_LEVEL, cfg.FPN.RPN_MAX_LEVEL + 1)):
                    ...
                    # print('loss_rpn_cls_fpn%d' % lvl, return_dict['losses']['loss_cls_fpn%d' % lvl], flush=True)
                    # print('loss_rpn_bbox_fpn%d' % lvl, return_dict['losses']['loss_bbox_fpn%d' % lvl], flush=True)
            else:
