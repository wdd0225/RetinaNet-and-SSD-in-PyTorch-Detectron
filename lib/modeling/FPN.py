...
def fpn_ResNet50_conv5_body():
    '''
    ResNet.ResNet50_conv5_body: returns a Generic ResNet components ResNet_convX_body((3, 4, 6, 3))
        which has res1 to res4
    fpn_level_info_ResNet50_conv5(): information var of levelInfo format with:
        blobs=('res5_2_sum', 'res4_5_sum', 'res3_3_sum', 'res2_2_sum'),
        dims=(2048, 1024, 512, 256),
        spatial_scales=(1. / 32., 1. / 16., 1. / 8., 1. / 4.)
    fpn builds the net based on above information
    '''
    return fpn(
...
class fpn(nn.Module):
    ...
    def __init__(self, conv_body_func, fpn_level_info, P2only=False):
        ...
        if cfg.FPN.EXTRA_CONV_LEVELS and max_level > HIGHEST_BACKBONE_LVL: # max_level = 7, backbone_lvl = 5
            self.extra_pyramid_modules = nn.ModuleList()
            dim_in = fpn_level_info.dims[0]
            for i in range(HIGHEST_BACKBONE_LVL + 1, max_level + 1): 
                self.extra_pyramid_modules.append(nn.Conv2d(dim_in, fpn_dim, 3, 2, 1))
#                 (
...
#                 )
    ...
    def forward(self, x):
        ...
        for i in range(1, self.conv_body.convX): # 5
        ...
        if hasattr(self, 'extra_pyramid_modules'):
            ...
            blob_in = self.extra_pyramid_modules[0](blob_in)

            fpn_output_blobs.insert(0, blob_in)
...
class fpn_rpn_outputs(nn.Module):
    ...
    def forward(self, blobs_in, im_info, roidb=None):
        '''K_min and k_max indicate the level to fetch for FPN'''
        k_max = cfg.FPN.RPN_MAX_LEVEL  # coarsest level of pyramid
        k_min = cfg.FPN.RPN_MIN_LEVEL  # finest level of pyramid
        ...
        for lvl in range(k_min, k_max + 1):
            ...
            if not self.training or cfg.MODEL.FASTER_RCNN:
                ...
                '''Each level has a GenerateProposalsOp layer object, so this line is calling forward function!'''
...
def fpn_level_info_ResNet50_conv5():
    '''resX_Y_sum, X is the block num, Y is block_count[X] - 1'''
    return FpnLevelInfo(
        blobs=('res5_2_sum', 'res4_5_sum', 'res3_3_sum', 'res2_2_sum'),
        dims=(2048, 1024, 512, 256),
        spatial_scales=(1. / 32., 1. / 16., 1. / 8., 1. / 4.)
