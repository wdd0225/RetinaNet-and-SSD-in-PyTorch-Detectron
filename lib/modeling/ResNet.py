...
class ResNet_convX_body(nn.Module):
    def __init__(self, block_counts):
        super().__init__()
        self.block_counts = block_counts # (3, 4, 6, 3)
        self.convX = len(block_counts) + 1 # 5
        self.num_layers = (sum(block_counts) + 3 * (self.convX == 4)) * 3 + 2 # 16 + 2 = 18

        self.res1 = globals()[cfg.RESNETS.STEM_FUNC]() # 'basic_bn_stem'
        dim_in = 64
        dim_bottleneck = cfg.RESNETS.NUM_GROUPS * cfg.RESNETS.WIDTH_PER_GROUP # 1 * 64 = 64
        '''64, 256, 64, 3, 1, 1'''
...
def add_residual_block(inplanes, outplanes, innerplanes, dilation, stride):
    ...
    '''dim_in, 256, dim_bottleneck, dilation=1, stride_init=1'''
    '''64, 256, 64, 1, 1'''
    if stride != 1 or inplanes != outplanes:
        shortcut_func = globals()[cfg.RESNETS.SHORTCUT_FUNC] # 'basic_bn_shortcut'
        downsample = shortcut_func(inplanes, outplanes, stride)
    else:
        downsample = None

    trans_func = globals()[cfg.RESNETS.TRANS_FUNC] # 'bottleneck_transformation'
...
def basic_bn_stem(): 
    return nn.Sequential(OrderedDict([
        ...
        ('bn1', mynn.AffineChannel2d(64)), # this step seems to change the scale from ~30 to ~2
