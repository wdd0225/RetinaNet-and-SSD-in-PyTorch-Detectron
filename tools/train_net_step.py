...
from modeling.model_builder import Generalized_RCNN, RetinaNet
...
def main():
    ...
    '''
    config is from core.config
    it feels like this script can only be trained with coco2017'''
    ...
    cfg.SOLVER.STEPS = list(map(lambda x: int(x * step_scale + 0.5), cfg.SOLVER.STEPS)) # list([0, 1000, 60000]) #
    ...
    dataiterator = iter(dataloader) # this step calls __getitem__
    ...
    if cfg.RPN.RPN_ON:
        ...
        model = mynn.DataParallel(model, cpu_keywords=['im_info'], # no rpn means no roidb
                         minibatch=True)
    ...
    CHECKPOINT_PERIOD = int(cfg.TRAIN.SNAPSHOT_ITERS / cfg.NUM_GPUS) #2500

    # Set index for decay steps
    '''seems like for different decay step size usage'''
